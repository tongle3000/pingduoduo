...
export class ProductFooterComponent implements OnInit {
  ...
  // 直接购买
  handleDirectBuy(variants: ProductVariant[]) { } // 这 2 个 click 事件,都要把数据都放进去.
  ...
  handleGroupBuy(variants: ProductVariant[]) {
    ...
    fromSubmitted.subscribe(ev => {
      console.log('00000fromSubmitted: ', ev);
      // tap(ev => console.log('fromSubmitted: ', ev));
      this.dialogService.saveData(ev);
      this.router.navigate(['/orders', 'confirm']); // 传到这个路由
    });
